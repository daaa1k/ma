...
LINUX_HASH=$(sha256_of_url "${BASE_URL}/${APP}-linux-x86_64")
MACOS_HASH=$(sha256_of_url "${BASE_URL}/${APP}-macos-aarch64")
...
FORMULA="HomebrewFormula/${APP}.rb"

sedi "s/version \"[^\"]*\"/version \"${VERSION}\"/" "$FORMULA"
sedi "s|sha256 \"[a-f0-9]*\" # macos-aarch64|sha256 \"${MACOS_HASH}\" # macos-aarch64|" "$FORMULA"
sedi "s|sha256 \"[a-f0-9]*\" # linux-x86_64|sha256 \"${LINUX_HASH}\" # linux-x86_64|" "$FORMULA"
...
if gh repo view "$TAP_REPO" >/dev/null 2>&1; then
  ...
  cp "$FORMULA" "$TAP_DIR/Formula/${APP}.rb"
  ...
  echo "  Then copy ${FORMULA} to Formula/${APP}.rb in the tap repo"
fi
